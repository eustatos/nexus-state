...
[Перейти к полной теории →](#theory)
[Перейти к практике →](#practic)

## Введение

Time-travel debugging — это возможность **сохранять снимки состояния** приложения и **перемещаться между ними**. Традиционно он ассоциируется с инструментами отладки, такими как Redux DevTools. Однако пользователи современных приложений (Figma, Google Docs, VS Code) ожидают функциональности перемещения между состояниями и от форм и веб-приложений.
...
2.  **Практика (Часть 2):** Туториал по реализации пользовательского time-travel с использованием **Nexus State**.

**Демо:** [Попробовать редактор](https://codesandbox.io/p/devbox/ntfhhj)
...
Time-travel debugging — это возможность **сохранять снимки состояния** приложения и **перемещаться между ними**.
...
![Pattern time-travel: snapshot, delta](https://habrastorage.org/webt/tj/rr/-l/tjrr-lzulkdhwg9pbtz4inacgjk.png)
...
<anchor>time-travel-patterns</anchor>
...
[Codesandbox](https://codesandbox.io/p/devbox/ntfhhj?embed=1&file=%2Fsrc%2Fstore%2FtimeTravel.ts)
[Preview](https://ntfhhj-3005.csb.app/)
...
![Nexus State time-travel demo architecture](https://habrastorage.org/webt/_c/d_/ad/_cd_ad8xvquxjtqsqx82ig5do_a.png)
...
> **Связь с теорией:** Этот шаг готовит инфраструктуру для реализации паттернов [Snapshot vs Delta](#time-travel-patterns) и [Debounce](#debounce).
...
> **Связь с теорией:** Этот шаг реализует [гибридный паттерн Snapshot + Delta](#time-travel-patterns). Параметры `fullSnapshotInterval: 10` и `deltaSnapshots.enabled: true` напрямую соответствуют рекомендациям из теоретической части.
...
> **Связь с теорией:** Этот шаг реализует [паттерн Debounce](#debounce) с параметрами `delay: 1000ms` и `maxWait: 5000ms`, рекомендованными в теоретической части для сценария «Форма».
...
![Sequince diagramm debounce](https://habrastorage.org/webt/p2/tj/a9/p2tja9dgyu_uxw4ogsmuqznhmm4.png)
...
![Time-travel state machine](https://habrastorage.org/webt/jk/bj/uo/jkbjuoqy_ibzne7qdtlf5kknqg0.png)
...
![Timeline slider — горизонтальная полоса прогресса с точками снимков](https://habrastorage.org/webt/ea/lh/mr/ealhmrwnecnpafw_yreuev26x1o.png)
...
![SnapshotList — боковая панель со списком изменений](https://habrastorage.org/webt/a0/z7/ml/a0z7mlhq-cys-96yq7xbjumkimc.png)
...
> **Связь с теорией:** Этот шаг применяет [методологию измерения производительности](#performance) из теоретической части. Сравнивайте полученные метрики с целевыми показателями.
...
![Snapshot lifecycle](https://habrastorage.org/webt/bx/9l/ht/bx9lht7xsmgdlahjotjfz4p0nqs.png)
...
**Documentation:** See full API and usage examples in `apps/demo-editor/src/test/README.md`.
...
1. **Попробуйте демо-приложение**
   - [Editor Demo](https://codesandbox.io/p/devbox/ntfhhj)
   - Поиграйте с undo/redo, timeline slider, режимами shtorm и turbo, попробуйте playback.
...
- [Документация Nexus State](https://nexus-state.website.yandexcloud.net/) — руководство и API reference
- [Демо-приложение](https://codesandbox.io/p/devbox/ntfhhj) — интерактивная демонстрация
