...
export class AtomTracker extends BaseDisposable {
  ...
  track<Value>(atom: Atom<Value>, name?: string, ttl?: number): boolean {
    debugTimeTravel(`Tracking atom: ${atom.name}, id: ${atom.id?.toString()}, atoms.size: ${this.atoms.size}`);
    ...
    if (this.atoms.has(atom.id)) {
      ...
      debugTimeTravel(`Atom already tracked, reactivated!`);
      return true;
    }
  ...
  async performCleanup(): Promise<CleanupResult> {
    ...
    if (this.ttlConfig.logCleanups && removed > 0) {
      debugTimeTravel(`Cleaned up ${removed} atoms, freed ${freed} bytes in ${duration}ms`);
    }
  ...
  private async cleanupAtom(atom: TrackedAtom): Promise<boolean> {
    try {
      ...
      debugTimeTravel(`Failed to cleanup atom ${atom.name}:`, error);
      return false;
    }
  }
