    ...
    create: (schema: z.ZodType) => {
      ...
      return {
        ...
        validateField: async <K extends keyof Record<string, unknown>>(
          ...
          // Determine if context is ValidationContext or plain values
          const allValues =
            context && 'values' in context
              ? (context as ValidationContext).values
              : (context as Record<string, unknown>);
          // If allValues is not provided, we cannot validate properly
          if (!allValues || typeof allValues !== 'object') {
            // Fallback to validating just this field with a dummy object
            const tempValues = { [fieldName as string]: value };
            const result = await schema.safeParseAsync(tempValues);
            if (!result.success) {
              const issue = result.error.issues.find(
                (issue) => issue.path.join('.') === fieldName
              );
              if (issue) {
                return zodIssueToFieldError(issue);
              }
            }
            return null;
          }
