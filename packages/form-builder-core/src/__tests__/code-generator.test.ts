...
import { generateCode, toKebabCase, toPascalCase } from '@nexus-state/form-builder-core';
import type { FormSchema } from '@nexus-state/form-builder-core';
...
describe('Code Generator', () => {
  ...
  describe('generateCode', () => {
    ...
    it('should generate with yup schema library', () => {
      const result = generateCode(mockSchema, {
        framework: 'react',
        typescript: true,
        schemaLibrary: 'yup',
      ...
      expect(result).toBeDefined();
      expect(result.files.some(f => f.content.includes('yup'))).toBe(true);
    });

    it('should generate with dsl schema library', () => {
      const result = generateCode(mockSchema, {
        framework: 'react',
        typescript: true,
        schemaLibrary: 'dsl',
    ...
    it('should generate with styled-components', () => {
      const result = generateCode(mockSchema, {
        framework: 'react',
        typescript: true,
        schemaLibrary: 'zod',
    ...
    it('should generate Vue code with options API', () => {
      const result = generateCode(mockSchema, {
        framework: 'vue',
        typescript: false,
        schemaLibrary: 'zod',
    ...
    it('should generate Svelte code with script tag', () => {
      const result = generateCode(mockSchema, {
        framework: 'svelte',
        typescript: false,
        schemaLibrary: 'zod',
      ...
      expect(result).toBeDefined();
      // Svelte files should contain script tag or export statements
      expect(result.files.some(f => f.content.includes('<script') || f.content.includes('export'))).toBe(true);
    });
  });
});
