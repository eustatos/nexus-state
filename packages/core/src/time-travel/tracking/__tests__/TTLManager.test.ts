...
describe('TTLManager', () => {
  ...
  describe('isExpired', () => {
    ...
    it('should return true for stale atom', () => {
      const atom = createMockAtom('atom1', {
        lastAccessedAt: Date.now() - 400000, // Older than default TTL
      });
    ...
    it('should use createdAt when lastAccessedAt is missing', () => {
      const atom = createMockAtom('atom1', {
  ...
  describe('isIdle', () => {
    ...
    it('should return true for idle atom', () => {
      const atom = createMockAtom('atom1', {
        lastAccessedAt: Date.now() - 120000, // Older than default idleTimeout
      });
  ...
  describe('isStale', () => {
    ...
    it('should return true for stale atom', () => {
      const atom = createMockAtom('atom1', {
        lastAccessedAt: Date.now() - 200000, // Older than default staleTimeout
      });
  ...
  describe('getStatus', () => {
    ...
    it('should return stale for stale atom', () => {
      const atom = createMockAtom('atom1', {
        lastAccessedAt: Date.now() - 200000,
      });
  ...
  describe('updateStatuses', () => {
    it('should update statuses for multiple atoms', () => {
      const atom1 = createMockAtom('atom1', { lastAccessedAt: Date.now() });
  ...
  describe('getExpiredAtoms', () => {
    it('should return stale atoms', () => {
      const atom1 = createMockAtom('atom1', {
        lastAccessedAt: Date.now() - 400000,
      });
      const atom2 = createMockAtom('atom2', {
        lastAccessedAt: Date.now(),
      });
