...
describe('middleware plugin', () => {
  ...
  it('should apply beforeSet middleware', () => {
    const beforeSetMock = vi.fn((atom, value) => value * 2);
  ...
  it('should apply both beforeSet and afterSet middleware', () => {
    const beforeSetMock = vi.fn((atom, value) => value + 10);
    const afterSetMock = vi.fn();
  ...
  it('should allow beforeSet to modify value', () => {
    const beforeSetMock = vi.fn((atom, value) => {
      if (value < 0) return 0;
      return value;
    });
  ...
  it('should work with multiple middleware on same atom', () => {
    const logger1 = vi.fn((atom, value) => value + 1);
    const logger2 = vi.fn((atom, value) => value * 2);
  ...
  it('should work with string atoms', () => {
    ...
    const beforeSetMock = vi.fn((atom, value) => value.toUpperCase());
  ...
  it('should work with object atoms', () => {
    const objectAtom = atom({ count: 0 }, 'object');
    const beforeSetMock = vi.fn((atom, value) => ({ ...value, count: value.count + 1 }));
  ...
  it('should work with multiple atoms and different middleware', () => {
    ...
    const logger1 = vi.fn((atom, value) => value * 2);
    const logger2 = vi.fn((atom, value) => value + 10);
  ...
  it('should handle beforeSet returning undefined', () => {
    const beforeSetMock = vi.fn(() => undefined);
...
describe('middleware with core features', () => {
  ...
  it('should work with store isolation', () => {
    ...
    const logger1 = vi.fn((atom, value) => value * 2);
    const logger2 = vi.fn((atom, value) => value * 3);
